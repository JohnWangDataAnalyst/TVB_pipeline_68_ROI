...
EnvironSet_index = 0
fields = ["Object ID", "T1 MRI Data Path", "dWMRI Data Path", "fMRI(Bold) Data Path", "Working Directory"]
...
${rootPath}/${scripts_path}/FreeSurfer_dt-recon.sh\n",
...
def WDprocessing(entries):
    ...
    if WDprocessing_index == 0 or (WDprocessing_index == 1 and ans_wd.lower() in ["y", "yes"]):
        ...
        for key in entries:
            ...
            elif key != "Object ID" and not os.path.exists(entries[key].get()):
...
def sh_file_generation(entries, checks):
    ...
    subprocess.call(qsub_call % "TVB_pipe.sh", shell=True)
